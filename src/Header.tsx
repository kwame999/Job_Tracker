...
import type { HeaderProps, ProjectSettingProp } from './Types';
    ...
    <section className='bg-gray-50 border-b border-black/[0.06]'>
      ...
      <div className='flex py-3 px-8 justify-between items-center h-[60px]'>
        ...
        <div className='flex gap-4 outline-1 rounded-md outline-gray-300 px-1 py-0.5  dropshadow-sm'>
          <button
            className={`p-1 hover:bg-black/[0.08] rounded-md transition-colors flex justify-center items-center ${isPowerMode && 'bg-black/[0.08]'}`}
            onClick={handlePowerMode}
          >
            {!isPowerMode ? (
              <IconSet iconName='zen' size={18}></IconSet>
            ) : (
              <IconSet iconName='power' size={22}></IconSet>
            )}
          </button>
      ...
      <div
        className={`w-full overflow-hidden transition-all duration-500 ease-in-out border-y border-black/[0.03] ${
      ...
      >
        ...
        <div className={`w-full h-[120px] relative ${crntBanner}`}>
          <div
            className={`absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/asfalt-dark.png')]`}
          ></div>
        </div>
      </div>
        ...
        <ProjectsSettingsModal onClose={handleSwitch}>
          <div className='flex flex-col gap-6'>
            ...
            <div className='flex flex-col gap-4'>
              <label className='text-[13px] font-semibold text-black/50 capitalize tracking-wider'>
                Your Banner:
              </label>
              <fieldset className='flex flex-col gap-3'>
                {presetBanners.map((bannerClass, index) => (
                  ...
                  >
                    ...
                    />
                    <div
                      className={`w-full h-12 rounded-lg border border-black/5 ${bannerClass} shadow-sm`}
                    ></div>
                  </label>
                ))}
              </fieldset>
