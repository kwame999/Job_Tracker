...
const SideNav = ({ recentJobs }: SideNavProps) => {
  ...
  return (
    ...
    >
      ...
      >
        {isExpanded && (
          <h3 className='font-bold text-lg text-black/80'>JTrack</h3>
        )}
        <div
      ...
      {isExpanded && (
        <div className='mt-5 px-[18px] w-full animate-in fade-in slide-in-from-top-2 duration-400'>
          ...
          <div className='flex flex-col gap-1'>
            <Link
              to='/'
              className='hover:bg-gray-100 rounded-lg w-full p-2.5 text-[14px] flex gap-2 items-center'
            >
            ...
            <Link
              to='/chat'
              className='hover:bg-gray-100 rounded-lg w-full p-2.5 text-[14px] flex gap-3 items-center'
            >
          ...
          <nav onClick={(e) => e.stopPropagation()}>
            <ul className='flex flex-col gap-4'>
              <li className='text-[10px] font-bold text-black/30 uppercase tracking-[0.15em] mt-8'>
                Tracked Jobs:
              </li>
              {recentJobs.map((job, i) => (
                <li
                  key={i}
                  className='text-sm font-medium text-black/60 hover:text-black cursor-pointer truncate transition-colors'
                >
                  {job.company}
                </li>
              ))}
