...
const ChatPage = ({jobsData, isPowerMode}: ChatPageProps) => {
   ...
   async function getCoachReply(input: string, jobsData: JobType[]) {
    ...
    return(
          <section className={`flex-1 flex flex-col bg-gray-100 h-full justify-center`}>
            
                {/* Stats section*/}
                {!isPowerMode && <div className="flex justify-center gap-8 p-4 bg-gray-20">
                ...
                {messages.length === 1 && (
                    <div className="h-full flex flex-col items-center justify-center text-center animate-fade-in ">
                        <img src='src\assets\JTrack.png' alt="" width={!isPowerMode ? 100 : 80} className="drop-shadow-md" />
                    <h1 className={`${!isPowerMode ?'text-3x1 font-extrabold text-gray-800 mb-7 mt-8 tracking-wide' : 'text-xl font-bold mb-3 mt-2 tracking-wide'} `}> 
                                     { !isPowerMode ? 'Hi, Applicant' : 'Hello!'}</h1>
                    <p className="text-gray-400 max-w-sm">
                        
                        { !isPowerMode ? 'What can coach help you with today? I analyzed your board and ready when you are.' : 'What are we working on?'}
                    </p>
                    
                     {(!jobsData.length && !isPowerMode) && <button className="bg-black/3 mt-4 p-2 rounded-full hover:bg-black/8 transition-opacity outline-1 outline-black/15">
                                            <Link to='/' className="">
